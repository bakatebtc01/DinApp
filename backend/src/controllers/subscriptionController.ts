...
export const createTier = async (req: Request, res: Response) => {
    try {
        const { creatorWalletId, tierLevel, name, monthlyPrice, description, benefits, badgeIcon, badgeColor } = req.body;
        ...
        if (tierLevel < 1 || tierLevel > 3) {
            return res.status(400).json({ error: 'Tier level must be between 1 and 3' });
        }
        ...
        if (error.message.includes('duplicate')) {
            return res.status(409).json({ error: 'Tier level already exists for this creator' });
        }
...
export const subscribe = async (req: Request, res: Response) => {
    try {
        ...
        const subscription = await subscriptionService.subscribe(subscriberWalletId, tierId);
...
export const getMySubscriptions = async (req: Request, res: Response) => {
    try {
        ...
        const subscriptions = await subscriptionService.getMySubscriptions(subscriberWalletId);
...
export const getCreatorSubscribers = async (req: Request, res: Response) => {
    try {
        ...
        const subscribers = await subscriptionService.getCreatorSubscribers(creatorWalletId);
...
export const checkSubscription = async (req: Request, res: Response) => {
    try {
        ...
        const result = await subscriptionService.hasActiveSubscription(subscriberWalletId, creatorWalletId);
...
export const getCreatorSubscriptionEarnings = async (req: Request, res: Response) => {
    try {
        const { creatorWalletId } = req.params;

        const earnings = await subscriptionService.getCreatorSubscriptionEarnings(creatorWalletId);
        res.status(200).json({
            earnings,
            revenueSplit: { creatorShare: '55%', platformShare: '45%' },
        });
