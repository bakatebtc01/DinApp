...
export const onboardAgent = async (req: Request, res: Response) => {
    try {
        ...
        const profile = await agentService.onboardAgent(userId, licenseNumber, region);
...
export const processPayout = async (req: Request, res: Response) => {
    try {
        const { agentWalletId, userWalletId, amount, otp, pin, userPhone, userId } = req.body;

        const transactionId = await agentService.processPayout(
            agentWalletId, userWalletId, amount, otp, pin, userPhone, userId
        );
