...
export const generateQR = async (req: Request, res: Response) => {
    try {
        const { merchantWalletId, qrType, amount, currency, description, expiresInMinutes } = req.body;

        if (!merchantWalletId || !qrType) {
            return res.status(400).json({ error: 'Missing merchantWalletId or qrType' });
        }

        if (!['static', 'dynamic'].includes(qrType)) {
            return res.status(400).json({ error: 'Invalid qrType. Must be static or dynamic' });
        }

        if (qrType === 'dynamic' && !amount) {
            return res.status(400).json({ error: 'Amount is required for dynamic QR' });
        }
...
export const decodeQR = async (req: Request, res: Response) => {
    try {
        ...
        if (!qrCode) {
            return res.status(404).json({ error: 'Invalid, expired, or used QR code' });
        }
...
export const processPayment = async (req: Request, res: Response) => {
    try {
        ...
        if (!qrPayload || !payerWalletId || !payerUserId || !otp || !pin || !payerPhone) {
...
export const getAudioConfirmation = async (req: Request, res: Response) => {
    try {
        ...
        const audioMessage = await receiptService.getAudioConfirmationMessage(receiptId);
...
export const rateVendor = async (req: Request, res: Response) => {
    try {
        const { merchantWalletId, payerWalletId, transactionId, rating, comment } = req.body;

        if (!merchantWalletId || !payerWalletId || !transactionId || !rating) {
            return res.status(400).json({ error: 'Missing required fields' });
        }

        await vendorService.addRating(merchantWalletId, payerWalletId, transactionId, rating, comment);
