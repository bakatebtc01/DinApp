...
// QR Code routes
router.post('/qr/generate', authLimiter, (req: Request, res: Response, next: NextFunction) => {
...
// Payment routes
router.post('/pay', authLimiter, (req: Request, res: Response, next: NextFunction) => {
    paymentController.processPayment(req, res).catch(next);
});

// Receipt routes
router.get('/receipt/:receiptNumber', (req: Request, res: Response, next: NextFunction) => {
    paymentController.getReceipt(req, res).catch(next);
});

router.get('/receipt/:receiptId/print', (req: Request, res: Response, next: NextFunction) => {
    paymentController.getPrintableReceipt(req, res).catch(next);
});

router.get('/receipt/:receiptId/audio', (req: Request, res: Response, next: NextFunction) => {
    paymentController.getAudioConfirmation(req, res).catch(next);
});

// Vendor routes
router.get('/vendor/:merchantWalletId/reputation', (req: Request, res: Response, next: NextFunction) => {
    paymentController.getVendorReputation(req, res).catch(next);
});

router.post('/vendor/rate', authLimiter, (req: Request, res: Response, next: NextFunction) => {
