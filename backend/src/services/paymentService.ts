 ...
 */
export const processQRPayment = async (params: QRPaymentParams): Promise<PaymentResult> => {
    const {
        qrPayload,
        payerWalletId,
        payerUserId,
    ...
    try {
        ...
        if (!qrCode) {
            return { success: false, error: 'Invalid, expired, or already used QR code' };
        }

        // 2. Determine amount
        const amount = qrCode.qr_type === 'dynamic' ? qrCode.amount : providedAmount;
        if (!amount || amount <= 0) {
        ...
        const audioMessage = await receiptService.getAudioConfirmationMessage(receipt.id);
