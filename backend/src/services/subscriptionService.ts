  ...
  try {
    ...
    await client.query(
      ...
      [
        ...
        tier.creator_wallet_id,
        creatorAmount,
        `Subscription revenue: ${tier.name} (55%)`,
        newCreatorBalance,
      ],
    );
    ...
    logger.info(
      `New subscription: ${subscriberWalletId} -> ${tier.creator_wallet_id} (${tier.name})`,
    );
...
export const cancelSubscription = async (
  ...
  logger.info(
    `Subscription cancelled: ${subscriptionId} (immediate: ${cancelImmediately})`,
  );
  return res.rows[0];
};
 ...
 */
export const processRenewals = async (): Promise<{
  processed: number;
  failed: number;
}> => {
  const client = await pool.connect();
      ...
      try {
        ...
        if (parseFloat(wallet.balance) < amount) {
          ...
          await client.query(
            ...
            [
              ...
              new Date(
                sub.current_period_end.getTime() + 30 * 24 * 60 * 60 * 1000,
              ),
            ],
          );
        ...
        await client.query(
          ...
          [
            ...
            sub.subscriber_wallet_id,
            amount,
            `Subscription renewal: ${sub.tier_name}`,
            newSubscriberBalance,
          ],
        );
        ...
        await client.query(
          ...
          [
            ...
            sub.creator_wallet_id,
            creatorAmount,
            `Subscription revenue: ${sub.tier_name} (55%)`,
            newCreatorBalance,
          ],
        );
