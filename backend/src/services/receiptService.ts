 ...
 */
export const createReceipt = async (params: CreateReceiptParams): Promise<Receipt> => {
    const {
        transactionId,
        payerWalletId,
        payeeWalletId,
  ...
  `;
    ...
    logger.info(`Receipt created: ${receiptNumber} for transaction ${transactionId}`);
    return res.rows[0];
};
 ...
 */
export const getReceiptByTransaction = async (transactionId: string): Promise<Receipt | null> => {
 ...
 */
export const getReceiptByNumber = async (receiptNumber: string): Promise<Receipt | null> => {
 ...
 */
export const generatePrintableReceipt = async (receiptId: string): Promise<string> => {
 ...
 */
export const getAudioConfirmationMessage = async (receiptId: string): Promise<string> => {
    const sql = `SELECT * FROM receipts WHERE id = $1;`;
    const res = await query(sql, [receiptId]);
