 ...
 */
export const sendGift = async (params: SendGiftParams): Promise<GiftTransaction> => {
    const {
        sessionId,
        giftId,
        senderWalletId,
    ...
    try {
       ...
       VALUES ($1, $2, 'debit', 'gift', $3, $4, $5)`,
            [transactionId, senderWalletId, totalAmount, `Gift: ${gift.name} x${quantity}`, newSenderBalance]
        );
       ...
       VALUES ($1, $2, 'credit', 'gift', $3, $4, $5)`,
            [transactionId, creatorWalletId, creatorAmount, `Gift received: ${gift.name} x${quantity} (55%)`, newCreatorBalance]
        );
       ...
       VALUES ($1, $2, 'credit', 'platform_fee', $3, $4, $5)`,
            [transactionId, creatorWalletId, platformAmount, `Platform fee: ${gift.name} x${quantity} (45%)`, newCreatorBalance]
        );
      ...
      RETURNING *`,
            [
                sessionId, giftId, senderWalletId, creatorWalletId,
                quantity, gift.price, totalAmount, gift.currency,
                creatorAmount, platformAmount, transactionId, message || null
            ]
        );
       ...
       WHERE id = $1`,
        ...
        logger.info(`Gift sent: ${gift.name} x${quantity} | Creator: ${creatorAmount} | Platform: ${platformAmount}`);
 ...
 */
export const getCreatorEarnings = async (creatorWalletId: string): Promise<{
    totalGifts: number;
    totalValue: number;
