 ...
 */
export const initializeReputation = async (merchantWalletId: string): Promise<VendorReputation> => {
    const sql = `
 ...
 */
export const getReputation = async (merchantWalletId: string): Promise<VendorReputation | null> => {
 ...
 */
export const recordSuccessfulTransaction = async (merchantWalletId: string): Promise<void> => {
    const sql = `
 ...
 */
export const recordDisputedTransaction = async (merchantWalletId: string): Promise<void> => {
    const sql = `
    UPDATE vendor_reputation 
    SET 
...
export const addRating = async (
    ...
    VALUES ($1, $2, $3, $4, $5);
  `;
    await query(insertSql, [merchantWalletId, payerWalletId, transactionId, rating, comment || null]);
 ...
 */
export const flagVendor = async (merchantWalletId: string, reason: string): Promise<void> => {
    const sql = `
    UPDATE vendor_reputation 
 ...
 */
export const getTopVendors = async (limit: number = 10): Promise<VendorReputation[]> => {
    const sql = `
