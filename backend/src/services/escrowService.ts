...
export const lockFunds = async (
    ...
    `;
    await query(sql, [escrowWalletId, sourceWalletId, amount, purpose, referenceId]);

    logger.info(`Funds locked in escrow ${escrowWalletId}: ${amount} (Ref: ${referenceId})`);
...
export const releaseFunds = async (
    ...
    logger.info(`Funds released from escrow ${escrowWalletId} to ${destinationWalletId}`);
    return transactionId;
};
...
export const refundEscrow = async (
    ...
    if (!res.rows.length) throw new Error('Escrow record not found or already processed');
    ...
    logger.info(`Funds refunded from escrow ${escrowWalletId} to ${source_wallet_id}`);
    return transactionId;
};
