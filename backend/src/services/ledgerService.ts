...
export const executeTransfer = async (
  ...
  try {
    ...
    const lockSql = `
      ...
      FOR UPDATE;
    `;
    const lockedWallets = await client.query(lockSql, [
      fromWalletId,
      toWalletId,
    ]);

    const fromWallet = lockedWallets.rows.find(
    ...
    const isAllowed = await complianceService.checkVelocityLimits(
      fromWallet.user_id,
      amount,
    );
    if (!isAllowed) {
      throw new Error(
        "Transaction blocked: Velocity limit exceeded or wallet frozen",
      );
    }
    ...
    await complianceService.recordTransactionVelocity(
      fromWallet.user_id,
      amount,
    );

    logger.info(
      `Transfer complete: ${transactionId} | ${amount} from ${fromWalletId} to ${toWalletId}`,
    );
