 ...
 */
export const createQRCode = async (params: CreateQRParams): Promise<{ qrCode: QRCodeData; qrImage: string }> => {
    const {
        merchantWalletId,
        qrType,
        amount,
    ...
    const expiresAt = qrType === 'dynamic'
 ...
 */
export const decodeQRPayload = async (qrPayload: string): Promise<QRCodeData | null> => {
    try {
        ...
        if (qr.expires_at && new Date(qr.expires_at) < new Date()) {
            await query(`UPDATE qr_codes SET status = 'expired' WHERE id = $1`, [qr.id]);
            return null;
        }
 ...
 */
export const markQRAsUsed = async (qrId: string, transactionId: string): Promise<void> => {
    const sql = `
    UPDATE qr_codes 
 ...
 */
export const getQRCodesByMerchant = async (merchantWalletId: string): Promise<QRCodeData[]> => {
    const sql = `
